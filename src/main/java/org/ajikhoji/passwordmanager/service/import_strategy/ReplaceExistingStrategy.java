...
public final class ReplaceExistingStrategy implements ConflictResolutionStrategy {
    ...
    public void resolve(ImportAnalyzeResult result) {
        try {
            try {
                ...
                DbConfig.commit();
                result.setImportedRecordCount(result.getNewAccounts().size() + result.getConflictAccounts().size());
